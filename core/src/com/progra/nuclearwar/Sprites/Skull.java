...
public class Skull extends Enemy {
    ...
    public Skull(PlayScreen screen, float x, float y) {
        ...
        for(int i = 0; i<3;i++)
            frames.add(new TextureRegion(screen.getAtlas().findRegion("Skeleton"),i*13,0,13,19));
        walkAnimation = new Animation(0.4f,frames);
        stateTime=0;
        setBounds(getX(),getY(),13/NuclearWarGame.PPM,19/NuclearWarGame.PPM);
        setOnDestroy = false;
        destroyed = false;
    }
    ...
    public void update(float dt) {
        ...
        if (setOnDestroy && !destroyed) {
            ...
            destroyed = true;
           // setRegion(screen.getAtlas().findRegion("Skeleton"), 13, 9, 13, 8);
        } else if (!destroyed) {
    ...
    protected void defineEnemy() {
        ...
        body.createFixture(fixturedef).setUserData("skull");
